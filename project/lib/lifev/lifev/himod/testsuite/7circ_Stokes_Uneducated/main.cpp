...
*/               
        ...
        boost::shared_ptr<matrix_Type> sinPressMatrix( new matrix_Type( pressureMap ) );
        boost::shared_ptr<matrix_Type> cosPressMatrix( new matrix_Type( pressureMap ) );
        boost::shared_ptr<matrix_Type> sinNormMatrix( new matrix_Type( Map ) );
        boost::shared_ptr<matrix_Type> cosNormMatrix( new matrix_Type( Map ) );
        ...
        sinPressMatrix->setBlockStructure( block_pressRow, block_pressCol );
        *sinPressMatrix *= 0.0;
        cosPressMatrix->setBlockStructure( block_pressRow, block_pressCol );
        *cosPressMatrix *= 0.0;
        ...
        *cosSystemMatrix *= 0.0;
        ...
        // H1-norm matrices
        sinNormMatrix->setBlockStructure( block_row, block_col );
        *sinNormMatrix *= 0.0;
        cosNormMatrix->setBlockStructure( block_row, block_col );
        *cosNormMatrix *= 0.0;
...
/*   		HiModExporterEnsight exporter( std::string( "StokesUneducated" ),
		                                    mx, mr, mtheta, mp,
		                                    *MBsin,
		                                    uMeshSize, Nelements,
		                                    Ntimesteps, dt );
...
//    	        exporter.writeSolution( *u_old3Dsin_interpolated, *u_old3Dcos_interpolated, t_iter, Nelements );
    		    exporterVtk.writeSolution( std::string( "SteadyStokes" ), u_old3D, t_iter );
    	    ...
    	    HMcos.interpolate( u_old3D, u_old3Dcos_interpolated );
    	    ...
    	    HMsin.addStokesProblem( sinNormMatrix, nu, refMap, t, 1. );
    	    HMcos.addStokesProblem( cosSystemMatrix, nu, refMap, t, alpha );
    	    HMsin.addStokesProblem( cosNormMatrix, nu, refMap, t, 1. );
	    HMsin.pressureMassMatrix( sinPressMatrix, refMap );
	    HMcos.pressureMassMatrix( cosPressMatrix, refMap );
    	    himodchrono.stop();

            sinPressMatrix->spy( "sinPressMatrix" );
            cosPressMatrix->spy( "cosPressMatrix" );
    	    ...
    	    if (verbose)
    	    {
   		    ...
   		    HMcos.addBC( cosNormMatrix, cosRhs, BC, t, cosine );
   		    HMsin.addBC( sinNormMatrix, sinRhs, BC, t, sine );
   		    ...
   		    cosNormMatrix->globalAssemble();
   		    sinNormMatrix->globalAssemble();

                    sinNormMatrix->spy( "sinNormMatrix" );
                    cosNormMatrix->spy( "cosNormMatrix" );
		    ...
		    //This is the same but from an analytical function. This operation ca be performed by HMsin or HMcos because it does not involve modal functions.
		    ues_3D = HMsin.evaluateBase3DGrid( x_ues, r_ues, theta_ues, p_es, t );
		    ues_3D.spy("ues_3D");
		    ...
		    // Oss: HMsin is equivalent to HMcos because normL2 does not use modalbasis evaluations.
...
//		    exporter.writeSolution( *sinSolution, *cosSolution, t_iter + 1, Nelements );
		    exporterVtk.writeSolution( std::string( "SteadyStokes" ), solution_3D, t_iter + 1 );
		    exporterVtk.writeSolution( std::string( "Poiseuille" ), ues_3D, t_iter + 1 );
