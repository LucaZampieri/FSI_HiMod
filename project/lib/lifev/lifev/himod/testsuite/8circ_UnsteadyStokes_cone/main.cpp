...
#include <lifev/himod/tools/utilityUnsteadyPressureFunctionsCone.hpp>
...
int main (int argc, char** argv)
{
...
#endif
    {
   		...
   		for( UInt t_iter = 0; t_iter != Ntimesteps; ++t_iter )
		{
		    ...
		    MBsin->map()->evaluateAxialMap( uMeshSize, udof, R, dR );
		    MBcos->map()->evaluateAxialMap( uMeshSize, udof, R, dR );
			...
			*fCos_interpolated *= 0;
    	    ...
    	    HMcos.interpolate( u_old3D, u_old3Dcos_interpolated, 1 ); // IMPORTANT: in this case u_old3D*r is interpolated implicitly
