...
int main (int argc, char** argv)
{
...
#endif
    {
        ...
        Real R = dataFile( "himod/fluid/D", 1. );
        R /= 2;
        ...
        UInt nquadRho = MBcos->qrRho()->nbQuadPt();
        UInt nquadTheta = MBcos->qrTheta()->nbQuadPt();
...
/*   		
   		HiModExporterEnsight exporter( std::string( "NavierStokes" ),
		                                    mx, mr, mtheta, mp,
		                                    *MBsin,
		                                    uMeshSize, Nelements,
		                                    1, dt );
   		...
   		for( UInt t_iter = 0; t_iter != Ntimesteps; ++t_iter )
		{
			...
			*fCos_interpolated *= 0;
			...
			if( t_iter == 0 )
    	    {
    	        ...
    	        u_kMinusOneSin = *u_old3Dsin_interpolated;
    	        u_kMinusOneCos = *u_old3Dcos_interpolated;
    	        
//    	        exporter.writeSolution( u_kMinusOneSin, u_kMinusOneCos, t_iter, Nelements );
    		    exporterVtk.writeSolution( std::string( "NavierStokes" ), u_old3D, t_iter );
		    ...
		    // u_old3D is initialized before the for-loop and is updated at the end of the t-for loop from solution_3D
		    u_old3D *= alpha;
			
    	    HMsin.interpolate( u_old3D, u_old3Dsin_interpolated ); // IMPORTANT: in this case u_old3D*r is interpolated implicitly
    	    ...
    	    *cosRhs *= 0;
		    ...
		    {
				++PFiter;
//				u_kMinusOne3D is initialized before the time for-loop and is updated at the end of the while-loop.
		
				// we start a chrono to understand the time needed for the assembling
				LifeChrono himodchrono;
				himodchrono.start();
				// Fill the matrix in
			    HMsin.addNavierStokesProblem( sinSystemMatrix, nu, refMap, t, alpha, u_kMinusOneSin );
			    HMcos.addNavierStokesProblem( cosSystemMatrix, nu, refMap, t, alpha, u_kMinusOneCos );
...
*/		      		    
			    ...
			    test = HMsin.normL2( err_3D, "velocity" ) / HMsin.normL2( solution_3D, "velocity" );
//			    test = HMsin.normL2( err_3D, "velocity" ) / HMsin.normL2( ues_3D, "velocity" );
			    std::cout << "Test quantity PF iteration #" << PFiter << " : " << test << std::endl;
			    // update FIXED POINT solution
			    u_kMinusOne3D = solution_3D;
			    u_kMinusOneSin = *sinSolution;
			    u_kMinusOneCos = *cosSolution;
...
//    	        exporter.writeSolution( u_kMinusOneSin, u_kMinusOneCos, t_iter + 1, Nelements );    	    
            exporterVtk.writeSolution( std::string( "NavierStokes" ), solution_3D, t_iter + 1 );
		}
    }
