...
#ifndef __HIMODEXPORTERPARAVIEW_HPP__
#define __HIMODEXPORTERPARAVIEW_HPP__
...
public:
    ...
    HiModExporterParaview( const NSModalSpaceCircular& MB,
    ...
    HiModExporterParaview( const ModalSpaceCircular& MB,
						const Real& h, const UInt& Nelements ) :
					    M_uMeshSize( h ), M_Nelements( Nelements ),
					    M_R( MB.Rho() ),
           				M_quadrulerho( new QuadratureRule( MB.qrRho() ) ),
           				M_quadruletheta( new QuadratureRule( MB.qrTheta() ) ) {};
*/           											
