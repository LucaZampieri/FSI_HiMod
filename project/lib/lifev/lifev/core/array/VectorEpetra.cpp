...
{
    Int lrow = blockMap().LID (static_cast<int>(row));
...
{
    return blockMap().LID (static_cast<int>(row)) >= 0;
}
                       ...
                       const UInt          column )
{
    ...
    {
        lid1 = vector.Map().LID (static_cast<int>(gids[i] + offset1));
        lid2 = blockMap().LID (static_cast<int>(gids[i] + offset2));
