    ...
    for (UInt ie = 0; ie < M_originalMesh->numElements(); ++ie)
    {
        for (UInt ifacet = 0; ifacet < M_elementFacets; ++ifacet)
        {
            ...
            if (vol != NotAnId)
            {
                ...
                {
                    myFacetRep = ( (M_interfaceMap->LID (static_cast<int>(M_originalMesh->facet (facet).point (ipoint).id()) )
                                    /* first is fluid */ == -1) &&
                                   (M_interfaceMapRep->LID (static_cast<int>(M_originalMesh->facet (facet).point (ipoint).id()) )
                                    /* first is fluid */ != -1) );
                    myFacet = myFacet ||
                              (M_interfaceMap->LID (static_cast<int>(M_originalMesh->facet (facet).point (ipoint).id()) ) != -1);
                    if (myFacetRep)
                    {
                        ++count;
                    }
