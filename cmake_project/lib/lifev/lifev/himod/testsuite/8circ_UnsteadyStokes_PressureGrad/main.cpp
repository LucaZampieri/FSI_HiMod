...
#include <lifev/himod/tools/utilityUnsteadyPressureFunctions.hpp>
...
int main (int argc, char** argv)
{
...
#endif
    {
       ...
       Real PFmax = dataFile( "fixedPoint/PF_max", 1000 );
        ...
        boost::shared_ptr< NSModalSpaceCircular > MBsin( new NSModalSpaceCircular
                                                             ( R, dR, Theta, mx, mr, mtheta, mp, &refMap, &quadRho, &quadTheta ) );
        boost::shared_ptr< NSModalSpaceCircular > MBcos( new NSModalSpaceCircular
                                                             ( R, dR, Theta, mx, mr, mtheta, mp, &refMap, &quadRho, &quadTheta ) );
...
/*               
        MBcos->evaluateBasis( "Bessel",
                              "Bessel",
                              "Bessel",
                              "ChebyshevQuadraticShiftNatural",
                              cosine );
        MBsin->evaluateBasis( "Bessel",
                              "Bessel",
                              "Bessel",
                              "ChebyshevQuadraticShiftNatural",
                              sine );
...
/*        HiModExporterEnsight exporter( std::string( "PressureGrad" ),
                                         mx, mr, mtheta, mp,
                                         *MBsin,
                                         uMeshSize, Nelements,
                                         Ntimesteps, dt );
*/        
         HiModExporterVtk exporterVtk( *MBsin,
                                       uMeshSize, Nelements );
/*                HiModExporterParaview exporterParaview( std::string( "PressureGrad" ), *MBsin, 
		                              uMeshSize, Nelements, Ntimesteps, dt );
*/
         ...
         *cosSystemMatrix *= 0.0;
         ...
         HMsin.addStokesProblem( sinSystemMatrix, nu, refMap, t, alpha );
         HMcos.addStokesProblem( cosSystemMatrix, nu, refMap, t, alpha );
         himodchrono.stop();

         std::vector<Real> errL2vec_u( Ntimesteps, 0 );
         std::vector<Real> errL2vec_p( Ntimesteps, 0 );
         std::vector<Real> errRelL2vec_u( Ntimesteps, 0 );
         std::vector<Real> errRelL2vec_p( Ntimesteps, 0 );
...
*/		    
	    ...
	    // Note that this is not the only way to assemble the rhs, you can use the functor, for more details look test_addrhs.
...
/*    		if( t_iter == 0 )
    	    {
    	        HMsin.plainInterpolate( u_old3D, u_old3Dsin_interpolated ); // IMPORTANT: in this case plain u_old3D is interpolated
           	    HMcos.plainInterpolate( u_old3D, u_old3Dcos_interpolated );
//    	        exporter.writeSolution( *u_old3Dsin_interpolated, *u_old3Dcos_interpolated, t_iter, Nelements );
    		    
                exporterVtk.writeSolution( std::string( "UnsteadyStokes" ), u_old3D, t_iter );
//                exporterParaview.writeSolution( std::string( "PressureGrad" ), u_old3D, t_iter );
...
*/  		
		    // u_old3D is initialized to 0 and is updated at the end of the t-for loop from solution_3D
            u_old3D *= alpha;
		    
    	    HMsin.interpolate( u_old3D, u_old3Dsin_interpolated );
    	    HMcos.interpolate( u_old3D, u_old3Dcos_interpolated ); // IMPORTANT: in this case u_old3D*r is interpolated implicitly
    	    HMsin.interpolate( fxrho, frrho, fthetarho, fSin_interpolated, t );
    	    HMcos.interpolate( fxrho, frrho, fthetarho, fCos_interpolated, t );
            ...
            *cosRhs *= 0.0;
            ...
            boost::shared_ptr<vector_Type> cosRhs_noBC( cosRhs );
            boost::shared_ptr<vector_Type> sinRhs_noBC( sinRhs );
   			
    	    // now that f is actually defined on the himodspace it is possible to add it to the rhs
    	    HMsin.addrhs( sinRhs_noBC, fSin_interpolated, u_old3Dsin_interpolated );
    	    HMcos.addrhs( cosRhs_noBC, fCos_interpolated, u_old3Dcos_interpolated );
    	    rhschrono.stop();
...
/* The matrix is constant in time, except for BC
            // we start a chrono to understand the time needed for the assembling
            LifeChrono himodchrono;
            himodchrono.start();
            // Fill the matrix in
...
*/
            ...
            cosRhs = cosRhs_noBC;
            sinRhs = sinRhs_noBC;
            HMcos.addBC( cosSystemMatrix, cosRhs, BC, t, cosine );
            HMsin.addBC( sinSystemMatrix, sinRhs, BC, t, sine );
...
//            exporter.writeSolution( *sinSolution, *cosSolution, t_iter + 1, Nelements );
            if( remainder(t_iter,tGap)==0 )
            {
                exporterVtk.writeSolution( std::string( "UnsteadyStokes" ), solution_3D, t_iter + 1 );
            }
//            exporterParaview.writeSolution( std::string( "UnsteadyStokes" ), solution_3D, t_iter + 1 );
    	    
            // THIS PART IS DROPPED BECAUSE TO COMPUTE THE NORM OF THE EXACT SOLUTION IS WAY TOO EXPENSIVE!!!
            vector_Type ues_3D( Map_3D, Unique );
            //This is the same but from an analytical function
            ues_3D = HMsin.evaluateBase3DGrid( x_ues, r_ues, theta_ues, p_es, (t_iter + 1)*dt );
            if( remainder(t_iter,tGap)==0 )
            {
