...
int main (int argc, char** argv)
{
...
#endif
    {
        ...
        ReferenceMap    refMap( Jr, Jtheta, Dr, Dtheta, Drtheta, Dthetar, Jacobian, inverseRhat, uSpace->map(), &quadRuleSeg32pt, &quadRuleFQSeg32pt );
        ...
        boost::shared_ptr< NSModalSpacePipe > MB( new NSModalSpacePipe( fR, fdR, Theta, mx, mr, mtheta, mp,
...
//        u_old3D *= 0.0;
           ...
           for( UInt t_iter = 0; t_iter != Ntimesteps; ++t_iter )
        {
            ...
            if( t_iter == 0 )
            {
                exporterVtk.writeSolution( std::string( "Solution" ), u_old3D, t_iter, 1 );
            }
            ...
            *f_interpolated *= 0;
            ...
            // also the rhs should have a block structure
               ...
               *Rhs *= 0.0;
               ...
               boost::shared_ptr<vector_Type> Rhs_noBC( Rhs );
            ...
            if( verbose )
            {
                std::cout << "Time of assemble rhs : " << std::scientific << rhschrono.diff() << std::endl;
                std::cout << "-- done! --" << std::endl;
            }
            ...
            SystemMatrix->matrixMarket( filename, false );
            filename.erase();
...
*/
            ...
            u_old3D = solution_3D;
            *u_old3D_interpolated = *Solution;

            filename = "Solution3D" + fileIter.str();
            solution_3D.matrixMarket( filename, false );
            filename.erase();
            fileIter.str( "" );
            exporterVtk.writeSolution( std::string( "Solution" ), solution_3D, t_iter + 1, 1 );
            ...
            std::cout << "L2 velocity u_h norm: " << normVel << std::endl;
            ...
            std::cout << "L2 pressure pes norm: " << normPressSol << std::endl;
            std::cout << "L2 pressure p_h norm: " << normPress << std::endl;
