...
#include <lifev/himod/tools/RealGeoData.hpp>
...
int main (int argc, char** argv)
{
...
#endif
    {
       ...
       UInt Ntimesteps = dataFile( "time/Nsteps", 20 );
       Real dt = dataFile( "time/dt", 0.05 );
       UInt tGap = dataFile( "time/tGap", 1 );
       Real alpha = 1./dt;
       // Real alpha = 0.;
        ...
        ******************************************************************************************/
        ...
        if (verbose)
        {
            std::cout << "Map definition and evaluation" << std::endl;
        }

        ReferenceMap refMap( "R.txt", "dxR.txt", "dtR.txt", uSpace->map(), udof, uMeshSize, &quadRuleSeg32pt, &quadRuleFQSeg32pt );
        ...
        ******************************************************************************************/
        ...
        BCScalarData    rOutflow( neu, rOut );
        BCScalarData    thetaOutflow( neu, thetaOut );
        ...
        for( UInt t_iter = 0; t_iter != Ntimesteps; ++t_iter )
        {
            ...
            */
            ...
            //while( test > tol && PFiter < PFmax )
            {
                ...
                *NSsystemMatrix = *SystemMatrix;
                ...
                //HMsin.addAdvection( NSsinSystemMatrix, u_old3Dpf );
                ...
                */
                ...
                //HMpostProc.setHMsolution( sinSolution, cosSolution );
                //solution_3D = HMpostProc.evaluateSolutionHMgrid( Map_3D );

                // Evaluation of the error with FEM solution
                /*if( t_iter+1 == 13 || t_iter+1 == 41 || t_iter+1 == 83 )
                {   
                    std::cout << "Evaluating HiMod solution on FEM nodes... " << std::endl; 
                    std::stringstream fileIter;
                    fileIter << t_iter+1;
            ...
            ******************************************************************************************/
            ...
            if( remainder(t_iter+1,tGap)==0 )
            {
                ...
                filename.erase();
                /*filename = "HMcos" + fileIter.str();
                (*cosSolution).matrixMarket( filename, false );
                filename.erase();
                filename = "HMsin" + fileIter.str();
                (*sinSolution).matrixMarket( filename, false );
                filename.erase();*/
                fileIter.str( "" );
