 ...
 */
#ifndef __MODALSPACEABSTRACT_HPP__
#define __MODALSPACEABSTRACT_HPP__
...
public:
    ...
    //@{
	ModalSpaceAbstract(UInt m): M_mtot(m) {};

    //! Destroys the monodimensional basis generators
    virtual ~ModalSpaceAbstract(){};
    //@}
    ...
    virtual    std::vector<Real> fourierCoefficients (const function_Type& g) const = 0;

    //! Compute The k-th Fourier coefficients of a function \f$f=f(x,y,z)\f$ in the point x
    virtual    Real fourierCoeffPointWise (const Real& x, const function_Type& f, const UInt& k) const = 0;
